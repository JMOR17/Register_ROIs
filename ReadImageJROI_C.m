function [sROI, matROIs] = ReadImageJROI_C(cstrFilenames, shape)
...
%   rmdir(strROIDir);
   ...
   for num_comp = 1:numel(cvsROIs)
       ...
       coords = ceil(cmp.mnCoordinates);
       coords(coords<=0) = 1;    
       coords(coords(:,1)>shape(1),1) = shape(1);
       coords(coords(:,2)>shape(2),2) = shape(2);
       coords_sqr = sub2ind(shape,coords(:,2),coords(:,1));
       mm = zeros(shape);
       mm(coords_sqr) = 1;
       mm = bwconvhull(mm);
       matROIs(:,:,num_comp) = mm;
